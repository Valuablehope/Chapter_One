...
router.post(
  ...
  [
    ...
    body('items.*.qty_ordered').isFloat({ min: 0.001 }).withMessage('Item quantity must be greater than 0'),
...
router.put(
  ...
  [
    ...
    body('items.*.qty_ordered').optional().isFloat({ min: 0.001 }).withMessage('Item quantity must be greater than 0'),
