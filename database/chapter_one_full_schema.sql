...
DO $$ BEGIN
    CREATE TYPE movement_reason AS ENUM (
        ...
        'cancelled', 'opening_stock', 'disposed'
    );
...
-- Dispose Reasons  (store_id NULL = system-wide seeded reason, set = per-store custom)
...
-- Stock Disposals  (header: one row per "Dispose items" submission)
...
-- Stock Disposal Items  (lines: product, qty, reason, snapshotted cost)
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_dispose_reason_global_name ON dispose_reasons (name) WHERE store_id IS NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uq_dispose_reason_store_name  ON dispose_reasons (store_id, name) WHERE store_id IS NOT NULL;
...
-- System dispose reasons (store_id IS NULL = global, non-deletable)
