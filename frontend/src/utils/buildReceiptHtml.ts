 ...
 * Designed to match the on-screen receipt preview exactly when printed on
 * thermal paper. No Tailwind or external CSS required.
 */
export function buildReceiptHtml(params: {
  ...
  if (settings?.receipt_header?.trim()) {
    headerHtml = `
      <div style="text-align:center;font-size:11px;white-space:pre-line;padding-bottom:6px;margin-bottom:6px;border-bottom:1px solid #000;line-height:1.4">
        ${esc(settings.receipt_header)}
      </div>`;
  } else {
    const name = receiptPrintTitle(settings?.name, settings?.code);
    let inner = `<div style="font-size:14px;font-weight:bold">${esc(name)}</div>`;
    if (settings?.address?.trim())
      inner += `<div style="font-size:11px;line-height:1.4">${esc(settings.address)}</div>`;
    if (settings?.phone?.trim())
      inner += `<div style="font-size:11px">${esc(settings.phone)}</div>`;
    headerHtml = `
      <div style="text-align:center;padding-bottom:6px;margin-bottom:6px;border-bottom:1px solid #000">
        ${inner}
      </div>`;
  }
  ...
  const metaHtml = metaRows
    .map(
      (r) => `
      <div style="display:flex;justify-content:space-between;gap:8px;line-height:1.5">
        <span style="flex:1;min-width:0">${esc(r.label)}</span>
        <span style="font-family:monospace;font-weight:600;white-space:nowrap;text-align:right">${esc(r.value)}</span>
      </div>`,
    )
    .join('');

  // Line items
  const ROW_BORDER = 'border-bottom:1px solid #e5e7eb';
  const lineHtml = lineRows
    .map((r, i) => {
      const last   = i === lineRows.length - 1;
      const color  = r.isReturn ? 'color:#dc2626;' : '';
      const border = last ? '' : ROW_BORDER;
      return `
        <tr style="${border}">
          <td style="padding:2px 4px 2px 0;vertical-align:top;${color}">
            ${esc(r.description)}
            ${r.isReturn ? '<span style="font-size:9px;font-weight:bold;text-transform:uppercase;margin-left:3px">(Return)</span>' : ''}
          </td>
          <td style="text-align:right;vertical-align:top;padding:2px 2px;white-space:nowrap;${color}">${esc(r.qty)}</td>
          <td style="text-align:right;vertical-align:top;padding:2px 2px;white-space:nowrap;${color}">${esc(r.price)}</td>
          <td style="text-align:right;vertical-align:top;padding:2px 0 2px 2px;white-space:nowrap;font-weight:500;${color}">${esc(r.total)}</td>
        </tr>`;
    })
    .join('');
  ...
  const totalsHtml = totalRows
    .map((r) => {
      if (r.style === 'strong') {
        return `
          <div style="display:flex;justify-content:space-between;gap:8px;font-weight:bold;font-size:14px;border-top:1px solid #000;margin-top:4px;padding-top:4px;line-height:1.5">
      ...
      if (r.style === 'strongSub') {
        return `
          <div style="display:flex;justify-content:space-between;gap:8px;font-weight:bold;font-size:14px;line-height:1.5">
            <span>${esc(r.label)}</span>
            <span style="font-family:monospace;white-space:nowrap;text-align:right">${esc(r.value)}</span>
          </div>`;
      }
          ...
          ${payments
            .map((p) => {
              ...
              return `
                <div style="display:flex;justify-content:space-between;gap:8px;line-height:1.5">
                  <span>${esc(label)}</span>
                  <span style="font-family:monospace;font-weight:500">${esc(formatCurrency(p.amount))}</span>
                </div>`;
            })
            .join('')}
          ${change > 0
            ? `<div style="display:flex;justify-content:space-between;gap:8px;font-weight:600;padding-top:2px;line-height:1.5">
  ...
  if (settings?.receipt_footer?.trim()) {
    footerInner = `<div style="font-size:11px;white-space:pre-line;line-height:1.4">${esc(settings.receipt_footer)}</div>`;
  } else {
    footerInner = `<p style="font-weight:600">${esc(t('receipt.thank_you_sale'))}</p>`;
  }
...
<body>
  ${headerHtml}
  <div style="margin-bottom:6px">${metaHtml}</div>
  <table style="width:100%;font-size:11px;margin-bottom:6px">
    <thead>
      <tr style="border-bottom:1px solid #000">
        <th style="text-align:left;font-weight:600;padding:2px 4px 2px 0">${esc(t('receipt.description'))}</th>
        <th style="text-align:right;font-weight:600;padding:2px 2px;width:2rem">${esc(t('receipt.qty'))}</th>
        <th style="text-align:right;font-weight:600;padding:2px 2px">${esc(t('receipt.price'))}</th>
        <th style="text-align:right;font-weight:600;padding:2px 0 2px 2px">${esc(t('receipt.total'))}</th>
      </tr>
    </thead>
  ...
  <div style="border-top:1px solid #000;padding-top:8px;margin-top:8px;text-align:center">
    ${footerInner}
    <p style="font-size:10px;color:rgba(0,0,0,0.7);margin-top:8px;padding-top:4px">${esc(t('receipt.by_cubiq'))}</p>
  </div>
</body>
</html>`;
}
